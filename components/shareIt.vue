			...
			>
				<div v-if="icons || config.icon">
					<slot :name="`${config.platform}-icon`">
						<font-awesome-icon :icon="[config.defaultIcon === 'envelope' ? 'fas' : 'fab', config.defaultIcon]" :size="config.size || iconSize || defaultIconSize" :style="{ color: config.color || config.defaultIconColor || config.defaultColor }" />
					</slot>
				</div>
				<div v-else>
					<share-button
					...
					>
						<slot :name="`${config.platform}-icon`">
							<font-awesome-icon :icon="[config.defaultIcon === 'envelope' ? 'fas' : 'fab', config.defaultIcon]" :size="config.size || defaultIconSize" />
						</slot>
		...
		openPopUp(config) {
			...
			switch (config.platform) {
				...
				case "email":
					...
					shareUrl += `?subject=Check out this NFT I just found 💎 🤯`;
